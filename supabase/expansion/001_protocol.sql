...
create table servos_v2.records(collection text not null,id text not null,version bigint not null check(version>0),data jsonb not null check(jsonb_typeof(master_data)='object'),archived boolean not null default false,primary key(collection,id));
