...
fn setup()->(tempfile::TempDir,rusqlite::Connection,Session){
    ...
    let session=login(&db,&user,"827193").unwrap();(dir,db,session)
}
...
#[test]fn version_conflicts_cannot_overwrite_records(){
    ...
    assert!(execute(&mut db,&s.token,update.clone()).is_err());update.target_version=Some(1);execute(&mut db,&s.token,update.clone()).unwrap();
